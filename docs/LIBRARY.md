...
This supported subset is published on npm `latest`. It remains a **0.x preview**,
...
The card contract carries numeric aggregates only and labels them
`client_supplied_aggregate`; schema validation is not server authentication.
...
Those checks and counter increments must be atomic in the future route.

This does **not** make an email route safe or shipped. A real route still needs
server-side rendering, atomic shared durable limits, waitlist lookup, minimal and
